...
- ✅ **Класс конфигурации приложения**:
  - Новый класс `AppConfiguration` в `Models/`
  - Содержит пути к БД, директорию данных, версию приложения
  - Регистрируется как singleton в DI
  - Используется во всех сервисах вместо передачи строк
...
- ✅ **Автоматическая миграция существующих баз данных**:
  ...
  - Экспорт данных в JSON перед миграцией
  - Создание двойного бэкапа (JSON + файл БД) с timestamp
  - Импорт данных из JSON через EF Core после применения миграций
  - Автоматическое удаление временных файлов после успешной миграции
...
- 🔄 **MauiProgram.cs**:
  - Создание и регистрация `AppConfiguration` как singleton
  - Фабричный метод для создания `LibraryService` с передачей пути из конфигурации
  - Добавлено логирование версии приложения

- 🔄 **YandexDiskPage.xaml.cs**:
  - Использует `AppConfiguration` вместо прямого создания пути к БД
  - Получает путь через DI из конфигурации
...
### Безопасность данных
- Двойной бэкап: JSON (`library.db.backup-{timestamp}.json`) + файл БД (`library.db.backup-{timestamp}`)
- JSON используется для импорта, файл БД - страховка на случай проблем
- Все операции логируются в Debug консоль с детальной информацией
- При ошибке миграции оба бэкапа остаются для ручного восстановления
- Автоматическое удаление временных файлов только после успешного завершения
