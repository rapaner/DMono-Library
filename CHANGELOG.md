...
## [2.9] - Исправление бага с сохранением книги

### Исправлено
- 🐛 **`ViewModels/AddEditBookViewModel.cs`** — при сохранении книги со статусом «Прочитано» дата завершения (`DateFinished`) больше не перезаписывается на `DateTime.Now`, если она уже была установлена. Оператор `=` заменён на `??=`, что сохраняет исходную дату при редактировании любых полей (полка, авторы и т.д.) без смены статуса
