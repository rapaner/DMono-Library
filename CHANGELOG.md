...
### Добавлено
- ✅ **Вычисляемая дата завершения чтения**:
  - Новое свойство `FinishedDateFromHistory` в `Library.Core/Models/Book.Computed.cs` определяет дату окончания по последней записи истории
  - На `BookDetailPage` отображается дата завершения из истории чтения (при наличии данных)
